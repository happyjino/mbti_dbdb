...
const EditDogInfo = () => {
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      setPetInfo(prev => ({
        ...prev,
        petProfile: URL.createObjectURL(file),
        selectedImage: file
      }))
    }
  };
  ...
  const editDogInfo = async () => {
    ...
    if (petInfo.selectedImage) {
      ...
      formData.append('petProfile', petInfo.selectedImage);

      const response = await fetch(`${domain}/pet/update/${petInfo.petId}`, {
        method: 'PATCH',
        headers: {
      ...
      const formData = new FormData();
      const imageUrl = petInfo.petProfile;
      ...
      <div className="main">
        ...
        <div className="input-box">
          <div className="image-box">
            <img className="pet-image" alt="펫이미지" src={petInfo.petProfile} onClick={handleImageBoxClick} />
